...
class Bot
{
    ...
    public $pool;
    public $hwidStats;
    public $hwidLimitMessage = 'HWID device limit exceeded';
    ...
    public function action()
    {
        switch (true) {
            ...
            case preg_match('~^/toggleHwidLimit$~', $this->input['callback'], $m):
                ...
                break;
            case preg_match('~^/setHwidLimit$~', $this->input['callback'], $m):
                $this->setHwidLimit();
                break;
            ...
            case preg_match('~^/resetXrUser (\d+)$~', $this->input['callback'], $m):
                ...
                break;
            case preg_match('~^/clearHwid ([^\s]+)(?:\s+(menu))?$~', $this->input['callback'], $m):
            ...
            case preg_match('~^/delHwid (.+?)(?:\s+(menu))?$~', $this->input['callback'], $m):
                ...
                break;
            case preg_match('~^/hwidDevices ([^\s]+)$~', $this->input['callback'], $m):
    ...
    public function getHwidStats(): array
    {
        if (!file_exists($this->hwidStats)) {
            foreach (['/configs/hwid_devices.json', '/root/configs/hwid_devices.json', '/config/hwid_devices.json'] as $legacyPath) {
    ...
    public function setHwidStats(array $stats): void
    {
        $dir = dirname($this->hwidStats);
        if (!is_dir($dir)) {
            mkdir($dir, 0777, true);
        }
        file_put_contents(
    ...
    public function handleHwidAccess(string $uid, string $email, bool $enforce, int $limit, bool $allowMissingHeader = false): void
    {
        ...
        $devices = $stats[$uid]['devices'] ?? [];

        if ($enforce && !isset($devices[$hwid]) && $limit > 0 && count($devices) >= $limit) {
            $this->denyHwidAccess();
        }
        ...
        $devices[$hwid] = [
            ...
            'last_seen'    => time(),
            'device_os'    => $_SERVER['HTTP_X_DEVICE_OS'] ?? ($entry['device_os'] ?? ''),
            'os_version'   => $_SERVER['HTTP_X_VER_OS'] ?? ($entry['os_version'] ?? ''),
            'device_model' => $_SERVER['HTTP_X_DEVICE_MODEL'] ?? ($entry['device_model'] ?? ''),
            'user_agent'   => $_SERVER['HTTP_USER_AGENT'] ?? ($entry['user_agent'] ?? ''),
    ...
    public function denyHwidAccess(): void
    {
        header('announce: base64:' . base64_encode($this->hwidLimitMessage));
    ...
    public function setHwidLimit()
    {
        ...
        $_SESSION['reply'][$r['result']['message_id']] = [
            'start_message' => $this->input['message_id'],
            'callback'      => 'saveHwidLimit',
            'args'          => [],
        ];
    }

    public function saveHwidLimit($count)
    {
        $count = (int) trim($count);
        $c     = $this->getPacConf();
        if ($count <= 0) {
            unset($c['hwid_limit_count']);
        } else {
            $c['hwid_limit_count'] = $count;
        }
        $this->setPacConf($c);
        $_SESSION['hwid_notice'] = $this->i18n('hwid limit warning');
        $this->xray();
    }
    ...
    {
        $c = $this->getPacConf();
        $c['hwid_limit_enabled'] = $c['hwid_limit_enabled'] ? 0 : 1;
        $enabled = !empty($c['hwid_limit_enabled']);
        if ($enabled && empty($c['hwid_limit_count'])) {
        ...
        if ($enabled) {
            ...
            if (!empty($this->input['callback_id'])) {
                $this->answer($this->input['callback_id'], $this->i18n('hwid limit warning'), true);
            }
        } elseif (!empty($this->input['callback_id'])) {
...
<code>example.org https://dns.adguard.com/dns-query 94.140.14.14</code>",
    ...
    public function clearHwidDevices($arg, $returnToMenu = false)
    {
        [$index, $clientIdHint] = $this->parseHwidMenuArg($arg);
        if ($index === null) {
            $this->xray();
            return;
        }
        ...
        $stats      = null;
        $clientData = $this->locateHwidClient($index, $clientIdHint, $stats);
        if ($clientData === null) {
            $this->xray();
            return;
        }

        [$client, $clientId, $statsKey] = $clientData;
        $stats ??= $this->getHwidStats();
        ...
        $menuArg = $this->buildHwidMenuArg($index, $clientId);
        if (!empty($_SESSION['hwid_device_map'][$menuArg])) {
            unset($_SESSION['hwid_device_map'][$menuArg]);
        }

        $this->hwidActionReturn($returnToMenu, $menuArg, $index);
    }
    ...
    public function deleteHwidDevice($arg, $returnToMenu = false)
    {
        ...
        if ($deviceToken === null) {
            ...
            $hwidEncoded     = $parts[2] ?? null;
            $clientArg       = $clientIdEncoded !== null ? $indexPart . '_' . $clientIdEncoded : $indexPart;
        } else {
        ...
        [$index, $clientIdHint] = $this->parseHwidMenuArg($clientArg);
        if ($index === null) {
            $this->xray();
            return;
        }
    ...
    public function hwidDevicesMenu($arg)
    {
        [$index, $clientIdHint] = $this->parseHwidMenuArg($arg);
        if ($index === null) {
            $this->xray();
        ...
        if (!empty($devices)) {
            ...
            $data[] = [
                [
                    ...
                    'callback_data' => "/clearHwid {$menuArg} menu",
                ],
            ];
        }
        $data[] = [
    ...
    private function hwidActionReturn(bool $returnToMenu, string $menuArg, int $index): void
    {
        if ($returnToMenu) {
    ...
    private function parseHwidMenuArg(string $arg): array
    {
        $parts = explode('_', $arg, 2);
        $indexPart = $parts[0] ?? '';
        if ($indexPart === '' || !ctype_digit((string) $indexPart)) {
            return [null, null];
        }
    ...
    private function buildHwidMenuArg(int $index, string $clientId): string
    {
        return $clientId !== '' ? $index . '_' . base64_encode($clientId) : (string) $index;
    }

    private function locateHwidClient(int $index, string $clientIdHint, ?array &$stats = null): ?array
    {
        $xray    = $this->getXray();
        ...
        $clientId  = $clientIdHint !== '' ? $clientIdHint : ($client['id'] ?? '');
        $statsKey  = $this->determineHwidStatsKey($client, $clientId, $stats);

        return [$client, $clientId, $statsKey];
    }

    private function determineHwidStatsKey(array $client, string $candidate, ?array &$stats = null): string
    {
        if ($stats === null) {
            $stats = $this->getHwidStats();
        }
        ...
        if (!empty($client['id']) && $client['id'] !== $candidate) {
            $possible[] = $client['id'];
        }
    ...
    public function xray($page = 0)
    {
        ...
        $text[] = "↓$td  ↑$tu";
        if (!empty($_SESSION['hwid_notice'])) {
            $text[] = '';
            $text[] = $_SESSION['hwid_notice'];
            unset($_SESSION['hwid_notice']);
        }
        $data[] = [
            [
        ...
        $hwidCount   = $p['hwid_limit_count'] ?: 1;
        $hwidEnabled = !empty($p['hwid_limit_enabled']);
        $data[]      = [
            [
                'text'          => $this->i18n('hwid limit') . ' ' . ($hwidEnabled ? ": $hwidCount" : $this->i18n('off')),
                'callback_data' => "/toggleHwidLimit",
            ],
            [
                'text'          => $this->i18n('set hwid limit'),
                'callback_data' => "/setHwidLimit",
            ],
        ];
    ...
    public function userXr($i)
    {
        ...
        $data[] = [
            [
                ...
                'callback_data' => "/hwidDevices $hwidCallback",
            ],
        ];
        $data[] = [
            [
